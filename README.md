...
- Application Project Directories (e.g., `/ims/OIM`, `/ims/MFA`).Application project directories contain the Bash and Python scripts to protect. Those scripts will consume the passwords at runtime but will never contain them.
...
- Runtime Secrets Directory (e.g., `/opt/credential_store`).This is the secure, permissions-restricted location on the server where the Ansible playbook places the necessaryfiles for runtime decryption. This is the **single point of interaction** for your application scripts when theyneed a password.
